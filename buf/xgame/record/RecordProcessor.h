...
#ifndef RECORDPROCESSOR_H_
#define RECORDPROCESSOR_H_
...
class RecordProcessor : public TcpProcessor, public MessageQueue
{
public:
    RecordProcessor(RecordServer* rs, socket_t s, const sockaddrin_t* sa, unsigned int id = 0)
        : TcpProcessor(s, sa, id), _gm(false), _type(SERVER_UNKNOWN), _rs(rs)
    {
        DEBUG("%s\n", __PRETTY_FUNCTION__);
    }
